...
class ArticleControllerTest {
    ...
    void deleteById_returns_response_OK() throws Exception {
        ...
        MvcResult result = mockMvc.perform(MockMvcRequestBuilders.delete("/articles/1")).andExpect(status().isOk()).andReturn();
        assertEquals(HttpStatus.OK.value(),result.getResponse().getStatus());
        verify(articleService,times(1)).deleteById(1);
    }
    @Test
    void deleteById_returns_response_NOT_FOUND() throws Exception {
        doThrow(ArticleNotFoundException.class).when(articleService).deleteById(anyInt());
        MvcResult result = mockMvc.perform(MockMvcRequestBuilders.delete("/articles/1")).andExpect(status().isNotFound()).andReturn();
